...
export default function Home() {
  ...
  const [soundEnabled, setSoundEnabled] = useState(true); // အသံ အဖွင့်/အပိတ် ထိန်းချုပ်ရန်
  ...
  // 🎵 Web Audio API ဖြင့် ပလောက်/ပလွတ် အသံများ ဖန်တီးခြင်း
  const playSound = useCallback((type) => {
    ...
    try {
      ...
      if (type === 'hover') {
        // "ပလောက်" (Soft Bubble Drop / Plop)
        osc.type = 'sine';
        osc.frequency.setValueAtTime(400, ctx.currentTime);
        osc.frequency.exponentialRampToValueAtTime(150, ctx.currentTime + 0.1);
        gainNode.gain.setValueAtTime(0.05, ctx.currentTime); // အသံတိုးတိုးလေး
        gainNode.gain.exponentialRampToValueAtTime(0.001, ctx.currentTime + 0.1);
        osc.start();
        osc.stop(ctx.currentTime + 0.1);
      } else if (type === 'click') {
        // "ပလွတ်" (Crisp Pop / Click)
        osc.type = 'triangle';
        osc.frequency.setValueAtTime(800, ctx.currentTime);
        osc.frequency.exponentialRampToValueAtTime(300, ctx.currentTime + 0.05);
        gainNode.gain.setValueAtTime(0.1, ctx.currentTime); // အသံနည်းနည်း ပိုကျယ်မည်
  ...
  // Click လုပ်သည့်အခါ အသံမြည်ပြီး Card ပေါ်လာစေရန်
    ...
    <div className="relative min-h-screen w-full flex items-center justify-center py-10 md:py-0">
      ...
      <div className="absolute top-4 right-4 md:top-6 md:right-12 z-50 flex items-center space-x-3 md:space-x-4 bg-[var(--glass-bg)] backdrop-blur-md border border-[var(--glass-border)] p-1.5 md:p-2 rounded-full shadow-sm font-note-text">
        <div className="flex gap-1 md:gap-2 text-xs md:text-sm">
          <button onClick={() => { setLang('MM'); playSound('click'); }} className={`px-3 py-1.5 rounded-full transition-all ${lang === 'MM' ? 'bg-blue-600 text-white' : 'hover:bg-gray-500/20 text-[var(--text-main)]'}`}>MM</button>
          <button onClick={() => { setLang('EN'); playSound('click'); }} className={`px-3 py-1.5 rounded-full transition-all ${lang === 'EN' ? 'bg-blue-600 text-white' : 'hover:bg-gray-500/20 text-[var(--text-main)]'}`}>EN</button>
        </div>
        ...
        <button 
          ...
          className="w-8 h-8 flex items-center justify-center rounded-full transition-all hover:bg-gray-500/20 text-[var(--text-main)]"
          title="Toggle Sound"
        >
          {soundEnabled ? (
        ...
        <button onClick={() => { setTheme(theme === 'light' ? 'dark' : 'light'); playSound('click'); }} className="px-3 py-1.5 rounded-full text-xs md:text-sm transition-all hover:bg-gray-500/20 text-[var(--text-main)]">
      ...
      <main className="relative z-10 w-full max-w-6xl flex flex-col md:flex-row gap-8 md:gap-10 p-4 md:p-10 mt-12 md:mt-0">
        ...
        <div className="w-full md:w-[40%] flex flex-col justify-start md:justify-center">
          <div className="text-left mb-6 md:mb-8">
            <h1 className="font-main-title gold-text-anim text-5xl md:text-6xl tracking-tight pb-2 cursor-default">
              Bhone Santi
            </h1>
          </div>
          ...
          <div className="flex flex-row md:flex-col overflow-x-auto md:overflow-visible space-x-4 md:space-x-0 md:space-y-3 pb-4 md:pb-0 hide-scrollbar">
            {sections.map((sec) => (
              <div key={sec.id} className="menu-wrapper">
                <button 
                  onMouseEnter={() => playSound('hover')} // မောက်စ်ထောက်လျှင် ပလောက် မြည်မည်
                  onClick={() => handleSectionClick(sec)} // ကလစ်နှိပ်လျှင် ပလွတ် မြည်မည်
              ...
              <div className="glass-widget">
                ...
                <div className="widget-bottom-bar mt-auto">
                  <div className="widget-nav">
                    <div className="nav-icon" onMouseEnter={() => playSound('hover')} onClick={() => playSound('click')}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 19.5v-15A2.5 2.5 0 0 1 6.5 2H20v20H6.5a2.5 2.5 0 0 1 0-5H20"/></svg></div>
                    <div className="nav-icon" onMouseEnter={() => playSound('hover')} onClick={() => playSound('click')}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/></svg></div>
                    <div className="nav-icon" onMouseEnter={() => playSound('hover')} onClick={() => playSound('click')}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg></div>
                  </div>
                  <div className="widget-action-btn" onMouseEnter={() => playSound('hover')} onClick={() => playSound('click')}>
